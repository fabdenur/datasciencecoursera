complete <- function(directory, id = 1:332) {
      ...
      ## loop (within loop) counts number of complete rows in all_data[[k]]
      x <- c()
      for (j in 1:nrow(all_data[[k]])) { 
            x[j] <- !any(is.na(all_data[[k]][j,]))
      }
      
     nobs[k] <- sum(x, na.rm=TRUE)
      }
